...
3. 客户端把数据推送给所有的副本，客户端可以以任意的顺序推送。每个 chunkserver 将会在一个内部的 LRU buffer 缓存这些数据，直到这些数据被使用或老化。通过将数据流与控制流解耦，我们可以通过基于网络拓扑调度昂贵的数据流来提高性能，而不管哪个 chunkserver 是 primary 的。我们会在 3.2 进一步讨论这一点。<font color=red>（图中步骤 3 好像看不出来是不是由客户端推给所有副本的）</font>
