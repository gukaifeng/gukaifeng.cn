...
![表 5 : 按操作大小(Size, %)对传输字节数划分。<br />对于读(Read)，size 是实际读、传输的数据总量，而不是请求的数据总量。如果读取尝试读取超过文件末尾的位置，则两者可能会有所不同，这在设计上在我们的工作负载中并不少见。](https://gukaifeng.cn/posts/gfs-lun-wen-yue-du-bi-ji/GFS_Table_5.png)
