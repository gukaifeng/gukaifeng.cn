...
![图 10：直接从一个配置切换到另一个配置是不安全的，因为各个服务器会在不同的时间切换。在这个例子中，集群从 3 个服务器增加到 5 个。不幸的是，有一个时间点，此时对于同一任期，可能有两个不同的领导者被选举出，一个持有旧配置中的大多数选票，一个持有新配置的多数选票。](https://gukaifeng.cn/posts/raft-lun-wen-yue-du-bi-ji/Raft_Figure_10.png)
...
![图 11：配置变更的时间线。虚线表示已创建但未提交的配置条目，实线表示最新提交的配置条目。](https://gukaifeng.cn/posts/raft-lun-wen-yue-du-bi-ji/Raft_Figure_11.png)

图 11 显示：领导者首先在其日志中创建配置条目 $C_{old,new}$，并将其提交给 $C_{old}$ 中的大多数和 $C_{new}$ 中的大多数。然后领导者创建条目 $C_{new}$ 并将其提交给新配置中的大多数。在任何时间点，新旧配置都不可以独立作出决策。