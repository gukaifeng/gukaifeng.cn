...
4. 投票者在投票给 $leader_U$ 时仍然存储该条目，因为每个涉及的领导者都包含该条目（根据假设。这里投票者会认为 Leader Completeness 属性成立，即新的领导者的日志中一定包含这个已提交的条目，不然都不会给 $leader_U$ 投票的。第 5.4.1 小节说过，Raft 保证从选举的那一刻起，每个新领导者上都存在之前任期内所有已提交的条目）。领导者永远不会删除条目，而追随者只会在与领导者冲突时删除条目。
