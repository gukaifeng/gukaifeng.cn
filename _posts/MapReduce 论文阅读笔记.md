...
导致 MapReduce 操作耗费的时间变长的常见原因之一是有一个“掉队者(straggler)”：一个机器花费了比平时更长的时间完成了计算中的最后几个 map 或 reduce 任务中的一个。掉队者出现的原因有很多。例如，一个机器的磁盘是坏的，频繁的出现一些可校正的错误（意思就是这个错误可以恢复，但恢复操作就需要消耗一定资源，拖慢任务执行速度），使得这个磁盘的读性能从 30 MB/s 降到了 1 MB/s。而集群调度系统可能把一些其他任务也分给这个台机器，就导致这些任务互相竞争 CPU、内存、本地磁盘或者是网络带宽，这样 MapReduce 代码的执行速度就更慢了。一个我们最近遇到的问题是一个机器初始化代码中的 bug，这个 bug 导致处理器 cache 变得不可用：受影响的机器的计算减慢了一百多倍。
...
读者不一定非得是从文件中读取数据，也可以是数据库，或者是映射到内存中的数据结构中。
