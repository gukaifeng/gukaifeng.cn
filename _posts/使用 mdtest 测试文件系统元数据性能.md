...
| 6    | `-u` | 每个任务一个工作目录。                                       |
| 7    | `-y` | 写入完成后 sync 文件。                                       |
| 8    | `-Y` | 在每个阶段后调用 sync 命令（包含在计时中；注意这会导致从你的结点 flush 所有 IO）。 |
| 9    | `-Z` | 打印时间，而不是速率（默认打印的只有速率）。                 |
| 10   | `-P` | 同时打印速率和时间（默认打印的只有速率）。                   |

| NO   | Optional arguments | 含义                                                         |
| ---- | ------------------ | ------------------------------------------------------------ |
| 1    | `-b`=1             | 层次目录结构的分支因子。                                     |
| 2    | `-d`=./out         | 运行测试的目录，可以有多个，用`@` 隔开，如 `-d=./out1@test/out2@~/out3`。 |
| 3    | `-e`=0             | 从每个文件读取的字节数。                                     |
| 8    | `-i`=1             | 测试将运行的迭代次数。                                       |
| 9    | `-I`=0             | 每个目录中的项目数。                                         |
| 10   | `-p`=0             | 迭代前延迟（以秒为单位）。                                   |
| 11   | `-w`=0             | 创建每个文件后写入每个文件的字节数。                         |
| 12   | `-z`=0             | 层次目录结构的深度。                                         |
